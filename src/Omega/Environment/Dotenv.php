...
use Omega\Environment\Exceptions\BadValueFormatException;
use Omega\Environment\Exceptions\InvalidKeyException;
use Omega\Environment\Exceptions\InvalidLineException;
use Omega\Environment\Exceptions\MissingEnvFileException;
use Omega\Environment\Exceptions\MissingVariableException;
use Omega\Environment\Exceptions\UnexpectedDirectoryException;
...
class Dotenv
{
    ...
    /**
     * Parse a string value from the .env file into its proper PHP type.
     *
     * This method handles common special cases such as "true", "false", "null", and "empty"
     * (with or without parentheses), converting them to their corresponding native PHP types.
     * It also detects numeric strings and casts them to int or float as appropriate.
     * If no conversion is needed, the original trimmed string is returned.
     *
     * @param string $value The raw value from the .env file.
     * @return mixed The parsed value with appropriate PHP type.
     */
    protected static function parseValue(string $value): mixed
    {
        ...
        if (is_string($parsed) && is_numeric($parsed)) {
            return $parsed + 0;
        }
