...
use Omega\Environment\Dotenv\ResultType\ResultInterface;
...
class EntryParser
{
    ...
    private const array REJECT_STATES = [
     ...
     */
    public static function parse(string $entry): ResultInterface
    {
        return self::splitStringIntoParts($entry)->flatMap(static function (array $parts) {
            ...
            return self::parseName($name)->flatMap(static function (string $name) use ($value) {
                /** @var AbstractResult<Value|null, string> $parsedValue */
     ...
     * @param string $line
     *
     * @return AbstractResult<array{string, string|null},string>
     */
    private static function splitStringIntoParts(string $line): AbstractResult
    {
        ...
        if ($result[0] === '') {
            /** @var AbstractResult<array{string, string|null},string> */
     ...
     */
    private static function parseName(string $name): AbstractResult
    {
     ...
     *
     * @param string $value
     * @return AbstractResult<Value, string>
     */
    private static function parseValue(string $value): AbstractResult
    {
        if (trim($value) === '') {
            /** @var AbstractResult<Value, string> */
            return Success::create(Value::blank());
        }

        return array_reduce(
            iterator_to_array(Lexer::lex($value)),
            static function (AbstractResult $data, string $token) {
     ...
     * @param int    $state
     * @param string $token
     * @return AbstractResult<array{string, bool, int}, string>
     */
    private static function processToken(int $state, string $token): AbstractResult
    {
        switch ($state) {
            case self::INITIAL_STATE:
                if ($token === '\'') {
                    /** @var AbstractResult<array{string, bool, int}, string> */
                ...
                if ($token === '"' || $token === '\\') {
                    ...
                    if (in_array($first, ['f', 'n', 'r', 't', 'v'], true)) {
                        ...
                        return Success::create(
                            [stripcslashes('\\' . $first) . Str::substr($token, 1), false, self::DOUBLE_QUOTED_STATE]
                        );
                    } else {
                        /** @var AbstractResult<array{string, bool, int}, string> */
